...
WriteBottomStatusLine:
      ...
      sta VRAM_Buffer1+2,x
      jsr GetWorldNumForDisplay  ;first the world number
      sta VRAM_Buffer1+3,x
...
PutLives:      sta VRAM_Buffer1+8
               jsr GetWorldNumForDisplay ;get world number or letter
               sta VRAM_Buffer1+19
               ldy LevelNumber
               iny
...
SetInitNTHigh: sty CurrentNTAddr_High   ;store name table address
               ...
               jsr RunGetAreaDataAddrs  ;get enemy and level addresses and load header
...
ContinueGame:
           jsr RunLoadAreaPointer    ;update level pointer with
...
.endif
NoLoadHW:
         jsr RunLoadAreaPointer    ;update area pointer to get world 1 or A
...
ResetTitle:   lda #$00                    ;reset game modes, disable
              sta OperMode                ;sprite 0 and screen output 
...
.ifdef ANN
DemoActionData:
  .byte $01, $00, $81, $01, $81, $42, $C2, $01
  .byte $81, $00, $C1, $41, $C1, $C2, $41, $C1
  .byte $42, $C2, $00

DemoTimingData:
  .byte $90, $70, $40, $18, $20, $10, $2C, $10
  .byte $1C, $70, $78, $08, $40, $20, $20, $40
  .byte $40, $20, $B0 ,$00
.else
DemoActionData:
...
CheckSaveData:
        ldx #$00                ;init counter
SChkLp: lda SaveHeader,x        ;check all seven bytes of the save data header
        ...
        inx
        cpx #$07                ;have we gone through all seven bytes?
        bcc SChkLp              ;if not, loop back
        rts                     ;otherwise save data is valid, leave
InitializeSaveData:
        ldx #$00                ;init counter
...
OverwritePrincessGfx:
		pha                    ;store A into stack
		lda #SoundBank         ;load sound bank since
		jsr SwitchPRGBank0     ;that's where the CHR data is
            lda #SoundBank+1
		jsr SwitchPRGBank1
            ldx #$00
		ldy #$00
		sty PPU_MASK           ;turn off rendering for good measure
		lda #>PrincessGfxOffset
		sta PPU_ADDRESS        ;load destination address into PPU
		lda #<PrincessGfxOffset
		sta PPU_ADDRESS
		pla                    ;retrieve A from stack
		bne LoadPrincessGfx    ;load princess graphics if nonzero
		lda #<(sm2char1_start+PrincessGfxOffset)
		sta $00
		lda #>(sm2char1_start+PrincessGfxOffset)
		sta $01
		jmp PrincessCHRLoop
...
.endif
EndCHRLoop: jmp LoadMainBank       ;load main bank afterwards
